...
if __name__ == "__main__":
    SVM = SVM()
    conf = SparkConf().setAppName("Support Vector Machines")
    ...
    lsvc = LinearSVC().setMaxIter(10).setRegParam(0.1)
    lsvc_model = lsvc.fit(training_data)
    predictions = lsvc_model.transform(test_data)
    end_time = time.time_ns()

    predictions_and_labels = predictions.select("prediction", "label").rdd.map(
